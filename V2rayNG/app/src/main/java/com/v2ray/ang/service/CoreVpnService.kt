...
class CoreVpnService : VpnService(), ServiceControl {
    ...
    private fun configureVpnService(): Boolean {
        ...
        try {
            ...
            // EKLENEN KISIM: Özel TTL kuralını root ile sisteme işleme
            val customTtl = MmkvManager.decodeSettingsString("pref_custom_ttl_value", "")
            if (!customTtl.isNullOrEmpty() && customTtl != "0") {
                try {
                    LogUtil.i(AppConfig.TAG, "StartCore-VPN: Özel TTL Değeri Uygulanıyor -> \$customTtl")
                    Runtime.getRuntime().exec(arrayOf("su", "-c", "iptables -t mangle -I POSTROUTING -j TTL --ttl-set \$customTtl"))
                } catch (e: Exception) {
                    LogUtil.e(AppConfig.TAG, "StartCore-VPN: TTL kuralı uygulanırken hata oluştu (Root izni yok veya magisk reddetti)", e)
                }
            }
    ...
    private fun stopAllService(isForced: Boolean = true) {
        ...
        // EKLENEN KISIM: Özel TTL kuralını sistemden temizleme (İnterneti normale döndürmek için)
        val customTtl = MmkvManager.decodeSettingsString("pref_custom_ttl_value", "")
        if (!customTtl.isNullOrEmpty() && customTtl != "0") {
            try {
                LogUtil.i(AppConfig.TAG, "StartCore-VPN: Özel TTL Değeri Temizleniyor -> \$customTtl")
                Runtime.getRuntime().exec(arrayOf("su", "-c", "iptables -t mangle -D POSTROUTING -j TTL --ttl-set \$customTtl"))
            } catch (e: Exception) {
                LogUtil.e(AppConfig.TAG, "StartCore-VPN: TTL temizleme hatası", e)
            }
        }
